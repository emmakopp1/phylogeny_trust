...
phylo_ie <- read.nexus(here("src/ancestral_reconstruction/IECoR_M1_CTMC_Gamma_1_Rate_For_All_Mgs_combined.trees"))
...
meanings_sets_ie <- read.csv(here("src/ancestral_reconstruction_/meanings_sets.csv"))
bounds_real_tb_by_sens_ie <- read.csv(here("src/ancestral_reconstruction/bounds_real_tb_by_sens.csv"))
bounds_real_tb_ie <- bounds_real_tb_by_sens[bounds_real_tb_by_sens$family == "IE", ]
...
path_out_ie <- here("output/results/res_ie_test.csv")
write.csv(
  x = data.frame(
    value = character(), 
    sens = character(), 
...
# Sino-Tibetan  ---------------------------------------------------------------
# Load BEAST posterior trees and thin sample 
phylo_st <- read.nexus(here("src/ancestral_reconstruction/st_ctmc-strict-fbd-uniform.trees"))
...
# Load and clean linguistic data 
Y_st <- read.nexus.data(here("src/ancestral_reconstruction/st.nex"))
Y_st <- lapply(Y_st, function(col) replace(col, col == "?", NA))
Y_st <- lapply(Y_st, as.numeric)
Y_st <- as.data.frame(Y_st)

# Load meaning boundaries and filter parameters
meanings_sets_st <- read.csv(here("src/ancestral_reconstruction/meanings_sets_st.csv"))
bounds_real_tb_by_sens <- read.csv(here("src/ancestral_reconstruction/bounds_real_tb_by_sens.csv"))
...
# Initialize output CSV
path_out_st <- here("output/results/res_st_test.csv")
write.csv(
  x = data.frame(
    value = character(),
    sens = character(),
