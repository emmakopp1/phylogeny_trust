...
list.dirs(here("data/simulated"), full.names = TRUE, recursive = FALSE) |> 
  map_df(~ list.files(.x, "\\.log", full.names = TRUE) |> 
        get_tracerlog_parameters()
      )

read.nexus.data(here("data/simulated/beast-data-sim-1/ctmc-strict-bd-1.trees"))
