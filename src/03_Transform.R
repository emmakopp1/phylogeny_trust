...
compute_upperbound_DT <- function(k, q, s) {
  k * sum(exp(-q  * s))
}
...
#V1
compute_inf_t_DT <- function(k, q, age, s, interval = c(0, 100), tol = 1e-6, maxiter = 10000) {
  uniroot(function(t) {
    compute_upperbound_DT(k, q, t, age, s) - 1
  }, extendInt = "yes", interval = interval, tol = tol, maxiter = maxiter)$root
}

#V2 
compute_inf_t_DT2 <- function(k, q, s, interval = c(0, 20), tol = 1e-6, maxiter = 10000) {
  compute_inf_t_DT2_varyq <- function(tt) {
    k * sum(exp(-q  * (tt - s))) - 1 
  }
  uniroot(Vectorize(compute_inf_t_DT2_varyq), extendInt = "yes", interval = interval, tol = tol, maxiter = maxiter)$root 
}
...
compute_upperbound_DS <- function(pi0, pi1, q, t, depth) {
  max(pi0,pi1) + sum(exp(-as.numeric(q) * (t - depth)))
}


#compute_upperbound_DS(foo_st$pi0,
#                      foo_st$pi1, 
#                      (foo_st$q * foo_st$mu * foo_st$clock_rate)/(2 * foo_st$pi0 * foo_st$pi1), 
#                      foo_st$t_R,
#                      filter(tipages_summary, family == 'ST')$depth)

# Compute the time threshold beyond which the upper bound of the probability
# of correctly inferring ancestral states falls below 1
compute_inf_t_DS <- function(pi0, pi1, q, t, depth, interval = c(0, 100), tol = 1e-6, maxiter = 10000) {
  uniroot(function(t) {
...
bounds_real_tb_by_sens <- tracelog_summary |>
  ...
  rowwise() |>
  mutate(
    ub_DS = compute_upperbound_DS(
      pi0 = pi0, 
      pi1 = pi1,
      q = (q * mu * clock_rate )/(2 * pi0 * pi1),
      t = t_R,
      depth = filter(tipages_summary, family == familyx)$depth
    ),
    inf_t_DS = compute_inf_t_DS(
      pi0 = pi0, 
      pi1 = pi1, 
      q = (q * mu * clock_rate)/(2 * pi0 * pi1), 
      t = t_R, 
    ...
    ub_DT = compute_upperbound_DT(
      k = n_cogsets, 
      q = (q * mu * clock_rate)/(2 * pi0 * pi1),
      s = filter(tipages_summary, family == familyx)$s
      ),
    inf_t_DT = compute_inf_t_DT2(
      k = k,
      q =  (q * mu * clock_rate)/(2 * pi0 * pi1),
      s =  filter(tipages_summary, family == familyx)$root_age - filter(tipages_summary, family == familyx)$s)
    ) |>
...
foo_kd <- filter(bounds_real_tb_by_sens, family == 'KD')
foo_st_by_sens <- filter(bounds_real_tb_by_sens, family == 'ST_by_sens')
foo_tea <- filter(bounds_real_tb_by_sens, family == 'TEA')


# t_max for ancestral reconstruction
# bantu
compute_inf_t_DS2(
  foo_bantu$pi0, foo_bantu$pi1, foo_bantu$q, foo_bantu$mu, foo_bantu$t_R,
  filter(tipages_summary, family == 'Bantu')$depth)

# bantu subset 
compute_inf_t_DS2(
  foo_bantu_subset$pi0, foo_bantu_subset$pi1, foo_bantu_subset$q, foo_bantu_subset$mu, foo_bantu_subset$t_R,
  filter(tipages_summary, family == 'Bantu_subset')$depth)

# bantu subset2
compute_inf_t_DS2(
  foo_bantu_subset2$pi0, foo_bantu_subset2$pi1, foo_bantu_subset2$q, foo_bantu_subset2$mu, foo_bantu_subset2$t_R,
  filter(tipages_summary, family == 'Bantu_subset2')$depth)

# indo-european
compute_inf_t_DS2(
  foo_ie$pi0, foo_ie$pi1, foo_ie$q, foo_ie$mu, foo_ie$t_R,
  filter(tipages_summary, family == 'IE')$depth)

# sino tibetan
compute_upperbound_DT(foo_st$pi0,
                      foo_st$pi1, 
                      (foo_st$clock_rate),
                      foo_st$t_R, 
                      filter(tipages_summary, family == 'ST')$depth)

compute_upperbound_DT(foo_st$k,
                      (1) / (2 * foo_st$pi0 * foo_st$pi1)  ,
                      foo_st$t_R,
                      filter(tipages_summary, family == 'ST')$depth,
                      filter(tipages_summary, family == 'ST')$s)

compute_inf_t_DS2(
  foo_st$pi0, 
  foo_st$pi1, 
  foo_st$clock_rate / (2 * foo_st$pi0 * foo_st$pi1) , 
  foo_st$mu, 
  foo_st$t_R,
  filter(tipages_summary, family == 'ST')$depth
  )
...
# kra-dai
compute_inf_t_DS2(
  foo_kd$pi0, foo_kd$pi1, foo_kd$q, foo_kd$mu, foo_kd$t_R,
  filter(tipages_summary, family == 'KD')$depth)

# sino tibetan by sens
tau_st <- matrix(nrow=nrow(foo_st_by_sens), ncol=2)
tau_st[,1] <- foo_st_by_sens$concept

for (k in 1:nrow(foo_st_by_sens)){
  tau_st[k,2] <- compute_inf_t_DS2(
    foo_st_by_sens$pi0[k], foo_st_by_sens$pi1[k], foo_st_by_sens$q[k], foo_st_by_sens$mu[k], foo_st_by_sens$t_R[k],
    filter(tipages_summary, family == 'ST_by_sens')$depth)
}

tau_st <- as_tibble(tau_st)
sum(as.numeric(tau_st$V2), na.rm=T)
...
sum(as.numeric(tau_tea$V2), na.rm=T)
sum(filter(bounds_real_tb_by_sens, family== 'TEA')$ub_DT, na.rm=T)
...
# compute t_max for heterogeneity
compute_upperbound_DT_ht <- function(t, familix){
  n_cogset <- filter(bounds_real_tb_by_sens, family == familix)$n_cogsets
  depth <- filter(tipages_summary, family == familix)$depth
  s <-  filter(tipages_summary, family == familix)$s
  
  somme <- 0
  for (i in 1:length(n_cogset)){
    k <- n_cogset[i]
    q <- filter(bounds_real_tb_by_sens, family == familix)$q[i] * filter(bounds_real_tb_by_sens, family == familix)$mu[i]
    
    somme <- somme + k * sum(exp(-q  * (t - depth - s)))
  }
  return(somme)
}

t_tea <- unique(filter(bounds_real_tb_by_sens, family == 'TEA')$t_R)
compute_upperbound_DT_ht(t_tea, 'TEA')

uniroot(
  function(t) {compute_upperbound_DT_ht(t, 'TEA') - 1}, 
  extendInt = "yes", 
  interval = c(0,20), 
  tol = 1e-6, maxiter = 10000)$root 
...
# maximum clade credibility trees for simulated tree ---------------------------

phylo_t5 <- read.nexus(here("data/simulated/beast-data-sim-5/ctmc-strict-bd-5.trees"))
M <- length(phylo_t5)
phylo_t5 <- phylo_t5[(0.2 * M):M]
t5_mcc <- maxCladeCred(phylo_t5)

write.tree(
  t5_mcc,
  here("data/simulated/beast-data-sim-5/mcc-5.tree")
)

phylo_t9 <- read.nexus(here("data/simulated/beast-data-sim-9/ctmc-strict-bd-9.trees"))
M <- length(phylo_t9)
phylo_t9 <- phylo_t9[(0.2 * M):M]
t9_mcc <- maxCladeCred(phylo_t9)

write.tree(
  t9_mcc,
  here("data/simulated/beast-data-sim-9/mcc-9.tree")
)
